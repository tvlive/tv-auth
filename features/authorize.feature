Feature: Authorize API User
  ...
  @XXXX
  Scenario: Unauthorized because the user does not exist
    When I GET the resource '/authorize/some.mark@tvlive.io/0123456789'
    Then the HTTP response is 'UNAUTORIZED'

  @XXXX
  Scenario: Unauthorized because the token does not exist
  ...
  @XXXX
  Scenario: Unauthorized because the token expired
    Given The users with credentials to use the API:
      | username                  | token      | date expaired       |
